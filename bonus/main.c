...
/*   Updated: 2023/12/31 00:10:27 by mateo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_len_conv(const char *str, int i)
{
	...
	while (ft_charinstr(str[i], PRECISION)) // needs dot!
	{
		len_conv++;
		i++;
	}
...
void	ft_putconv_0(t_conv *conv)
{
	...
	conv->width_num = 0;
	conv->precision = 0;
	conv->precision_num = 0;
	conv->spec = 0;
}

t_conv	*ft_putconv(char *word, int len)
{
	t_conv	conv;
	...
	if (ft_charinstr(*word, WIDTH))
	{
		conv->width = 1;
		conv->width_num = ft_atoi(word);
	}
	while (ft_charinstr(*word, WIDTH))
		word++;
	if (*word == '.' && ft_charinstr(*(word + 1), PRECISION))
	{
		conv->precision = 1;
		conv->precision_num = ft_atoi(++word);
	}
	while (ft_charinstr(*(word + 1), PRECISION))
		word++;
	if (ft_charinstr(*word, CONVERSIONS)
		conv->spec = *word;
	else
		return (0);
	return (conv);
